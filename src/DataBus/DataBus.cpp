...
void DataBus::publish(BusEventType type, const std::string &source,
...
DataBus::setup_response_waiter(const std::string &correlation_id) {
  std::lock_guard<std::mutex> lock(requests_mutex_);
  auto [it, inserted] =
      pending_requests_.emplace(correlation_id, std::promise<nlohmann::json>());
  ...
  for (auto &[corr_id, promise] : pending_requests_) {
    try {
      promise.set_exception(
          std::make_exception_ptr(std::runtime_error("DataBus stopped")));
    } catch (...) {
    }
  }
